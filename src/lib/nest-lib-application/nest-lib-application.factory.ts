...
import { Schema } from './nest-lib-application.schema';
...
export function main(schema: Schema): Rule {
    return (host: Tree, context: SchematicContext) => {
        ...
        return chain([
            createNrwlLibrary(options.rootDir, options.prefix, options.name, 'application'),
...
function normalizeOptions(host: Tree, options: Schema): NormalizedSchema {
    ...
    const applicationPath = buildSrcPath(defaultSourceRoot, options.name + '/application');
