...
import { Schema } from './nest-lib-domain.schema';
...
export function main(schema: Schema): Rule {
    return (host: Tree, context: SchematicContext) => {
        ...
        return chain([
            createNrwlLibrary(options.rootDir, options.prefix, options.name, 'domain'),
...
function normalizeOptions(host: Tree, options: Schema): NormalizedSchema {
    ...
    const domainPath = buildSrcPath(defaultSourceRoot, options.name + '/domain');
